...
func (s *httpServer) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	/*
	   ...
	   在v1.Router中，
	   首先，判断是否指定的Topic名称是否存在
	   如果存在，直接从nsqd.map[string]*Topic中去除特定名称对应的Topic
	   如果不存在，创建一个新的Topic，然后保存到nsqd.map[string]*Topic中

	   然后将http.Request.Body创建新的Message : NewMesage()

	   然后将新的Mesage m -> Topic.messageChanMsg

	   最后根据httpServer.doPub返回错误err并生成对应的Response
	*/
...
func (s *httpServer) v1Router(w http.ResponseWriter, req *http.Request) error {
	fmt.Printf("v1Router running .....\n")
	switch req.URL.Path {
	case "/pub":
		http_api.NegotiateVersionWrapper(
			w,
			req,
			//NegotiateVersionWrapper先调用下面的函数
			http_api.RequirePOST(
				req,
...
func (s *httpServer) doPUB(req *http.Request) (interface{}, error) {
	...
	fmt.Printf("Topic.PutMessage() is Returned\n")
	return "OK", nil
}
